...
int col=42;
int row = 8;
node[][] pos = new node[col][row];  // 2d Array of 42*8
float wid;
float ht;
List<park_space2> spot = new ArrayList<park_space2>();
PShape Node,Node_inner,/*Car,*/Lot;  // Lot is the main matrix shape of the layout
IntDict spaces;
String[] lines;
String end_position;
//PShape carr;
...
void setup()
{
  ...
  frameRate(30);  //Rendor frame rate
  
  Lot = createShape(GROUP);  // Create Matrix of Nodes  // And GROUP is an alias for a complex, nested geometric shape
  wid = width / col;  //wid -> Width of Each Node  && width -> width of window
  ht = height / row;  // ht -> Height of Each Node  && height -> height of window
  Node = createShape(RECT, 0, 0, wid, ht);  //Creates Nodes ... shape of one brick/node
...
void parkLot2(int x, int y, int num, int star_pos2) {
  ...
  for (int i = num-1; i >= 0; i--) {
    pos[x+i][y].obs = 1;//[y+j].obs = 1; // whichever are in te 
