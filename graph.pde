class node
{
  ...
  //node[] Final;
  int obs; // obs-> obstacle, if obs === 1, this node is an obstacle!
  int ps;
  node parent;  // it stores the previous node in the path given by A* algo!
  ...
  node(int I, int J){
  //side = width/col;]
  x = I;   // I -> the column pos, in this case I : [1,42] J:[1,8]
  y = J;  //J-> the row pos... 
  i = I*wid;    // wid-> width of one node * the X th position of node gives the x- coordinate of node
  //side = height/row;
  j = J*ht;
...
//Add Neighbour Nodes to Neighbour List
  ...
  void display(int a , int b , int c , int d )
  {
    ...
    if (this.obs==1)
    {
      ...
      //stroke(120,120,120);
      noStroke();
      fill(255,255,255,200);  // make this white
  ...
  {
    node q ;  // this is dummy variable as of now!
    ...
    open.add(this);  // we are adding the current brick to open
    while (open.size()>0)
    {
      float minOpen = open.get(0).f; // f might be some property, 
      q = open.get(0);
        for (int i = open.size() - 1; i >= 0; i -= 1)
        {   if (open.get(i).f <= minOpen) {   // it is filtering on the basis of min (F)
            minOpen = open.get(i).f;
            q = open.get(i);
           }
        }
      open.remove(open.indexOf(q)); // removing that min f holder brick
